...
## PATCH: paths now match the documented data/ subdirectory and are actually used.
...
## PATCH: read from the resolved, machine-independent path instead of a
## hard-coded absolute OneDrive path.
...
## PATCH: report how many respondents fall through to an unclassifiable (NA)
...
## PATCH: keep NA as NA instead of letting `%in%` silently fold unclassifiable
## respondents into the 0 (reference) category. Non-voters remain a genuine 0
## (that is the Table C1 vs C2 contrast); only truly unknown vote is dropped.
data <- data %>%
  mutate(
...
##
## PATCH: add validity filters on the demographics (previously unguarded). ISSP
## missing codes (negative / 9xxx) otherwise leak through: e.g. any non-1 `b1`
## (incl. missing) was being coded female = 1, and a missing-coded `b2` produced
## a nonsense age that entered every model as a control.
## VERIFY AGAINST CODEBOOK:
##   - b1 sex coding (assumed 1 = male, 2 = female);
##   - b2 is a full 4-digit birth year (range guard 1900-2010);
##   - b3 education: only the lower bound is guarded because the number of
##     categories is codebook-specific — set an upper cap if you have it.
df_base <- data %>%
  filter(b1 %in% 1:2) %>%
...
set.seed(2023)                             ## PATCH: seed the simulation-based parallel analysis
...
## PATCH: anchor the score sign (higher = more trust) so it is not flipped by
...
## ----------------------------------------------------------------------------
## PATCH: replace open-ended `> 0` guards with two-sided `between()` so upper
## "don't know"/missing codes cannot slip in. Ranges are INFERRED from the
## recoding arithmetic (`8 - q19` => 1-7 scale; `6 - q7_5` => 1-5 scale) and
## should be checked against the codebook.
df_illib <- df_base %>%
...
## ----------------------------------------------------------------------------
## PATCH: factor the recode into a reusable helper so df_auth and df_full stay
## in sync (previously the same recode was hand-copied into df_full, inviting
## drift). Range guard tightened to between(q04c, 1, 5).
...
add_populism <- function(df) {
  ...
  ## PATCH: anchor both PCA scores so they carry the same sign in df_pop and
  ## df_full (higher = more populist), making the ml3 vs ml5 coefficients
...
## ----------------------------------------------------------------------------
## PATCH: factor the recode into a reusable helper (shared with df_full).
...
##
## PATCH: reuse add_authoritarianism()/add_nativism()/add_populism() instead of
## re-typing each recode, so the definitions cannot drift from df_auth/df_nat.
df_full <- df_illib %>%
...
## ============================================================================
## PATCH (justification, not a code change): models are fit with glm(weights =
## weight_FINAL) and paired with HC0 sandwich SEs (Section 7). This yields the
## correct weighted point estimates but triggers the harmless "non-integer
## #successes" warning, and HC0 approximates rather than reproduces a design-
## based variance. If weight_FINAL is a full survey weight and design variables
## (PSU/strata) are available, survey::svyglm with a matching svydesign would
## give linearization SEs; HC0 is retained here for comparability with the
## published tables. Adjust if the journal requires design-based inference.
...
## ============================================================================
## PATCH: a single helper builds every OR table. It reports odds ratios with
## *exponentiated* 95% robust CIs (via ci.custom) instead of pairing an OR point
## estimate with a log-odds-scale SE, which was internally inconsistent in the
## original tables. Robust p-values (computed on the log-odds scale) still drive
...
## PATCH: build the CIs from the HC0 vcov so these bands are on the same robust
## footing as every other quantity in the paper (predict.glm's se.fit would use
## the model-based vcov instead).
...
## PATCH: seed immediately before the model (multinom uses random starts), raise
## maxit so the 6-outcome / 15-predictor model is not cut off at the default 100
## iterations, and assert convergence.
set.seed(2023)
mult_model <- multinom(
...
## PATCH: coefficients are relative-risk ratios (exp(beta)). We now also pass
## model-based p-values so the table shows statistical significance. NOTE: these
## SEs/p-values are on the log scale and are model-based, NOT HC0-robust —
## nnet::multinom is not supported by the sandwich estimator used elsewhere, so
## robust SEs would require a different route (e.g. a bootstrap). The p matrix is
## oriented to match exp(coef(mult_model)) (outcomes x terms).
...
## PATCH: pass the survey weights explicitly to avg_slopes(). marginaleffects
## does not necessarily inherit weight_FINAL from a multinom object, so without
## this the AMEs could be an UNWEIGHTED average of a weighted model. Compare the
## estimates with/without to confirm they move as expected.
...
## PATCH: same seed/maxit/convergence treatment as the main multinomial model.
set.seed(2023)
...
ame_df_robust <- avg_slopes(mult_model_robust, wts = "weight_FINAL") %>%   ## PATCH: weighted AMEs
...
## ============================================================================
## PATCH: freeze the environment used to produce these results. For a fully
## pinned dependency set, additionally run renv::init() / renv::snapshot() once
## and commit the resulting renv.lock alongside this script.
